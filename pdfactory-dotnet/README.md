...
- `PdfFactory.Client` — calls `/render` and returns PDF bytes
- `PdfFactory.Builders` — RazorLight HTML builders (invoice, report, etc.)
...
using PdfFactory;
using PdfFactory.Builders;

var services = new ServiceCollection();
services.AddPdfFactoryClient(options => options.BaseAddress = "http://localhost:8080/");
